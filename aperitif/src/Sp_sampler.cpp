...
map<uint32_t, double>/*vector<uint32_t>*/ Sp_sampler::random_path(int &path_length , int &num_paths , double alpha_sp_sampling) {
    ...
    if (uniform){
        ...
        if (sampling_kernel.optimized){
            ...
            std::tie(u, v) = non_uniform_sampling_linear(sampling_kernel,g->get_nn(),rng);
    ...
    for(int j = 0; j<num_paths_to_sample; j++){
        ...
        for(uint32_t u:path){
            if (uniform){
                ...
                path_map[u] = path_map[u] + sampling_kernel.uniform_denominator[u]/sampling_kernel.c_total; 
            }
        }
