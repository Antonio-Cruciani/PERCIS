...
for eps in epsilon_list
    for ds in datasets
        ...
        for i in 1:runs
            op = outpath *"uniform_vc_ss_"*string(j)*"_run_"*string(i)*".txt"
            ...
            args = `-u -f -v 10 -s $sampling_rate_ -o $op -t $tn $eps $delta $ps $gf`
            @info("----------------------------------------------------------------------------------")
            @info("Run Number $i")
            ...
            op_times = "uniform_vc_ss_"*string(j)*".txt"
...
for eps in epsilon_list
    for ds in datasets
        ...
        for i in 1:runs
            ...
            #println(ps)
            args = `-u -f -d -v 10 -s $sampling_rate_ -o $op -t $tn $eps $delta $ps $gf`
