...
exact_scores_path = "../../../datasets/scores/"
#sampling_rate_ = 2.3
sampling_rate_ = 0.0
tn = 64
directed = false
...
for denom in fractional
    for ds in datasets
        ...
        for i in 1:runs
            ...
            args = `-v 10 -s $sampling_rate_ -u -o $op -e $es -w $sampling_window -g $max_ss -t $tn $eps $delta $ps $gf`
            #println("ARGS ",args)
...
for denom in fractional
    for ds in datasets
        ...
        for i in 1:runs
            ...
            args = `-v 10 -s $sampling_rate_ -d -u -o $op -e $es -w $sampling_window -g $max_ss -t $tn $eps $delta $ps $gf`
            #args = `-u -v 1 -g $ss -o $op -t $tn $epsilon $delta $ps $gf`
            @info("----------------------------------------------------------------------------------")
            @info("Run Number $i")
            for line in eachline(`../percis/percis $args`)
